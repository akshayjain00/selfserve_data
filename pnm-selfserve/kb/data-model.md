...
| PNM-T-088 | `PICKUP_CITY_NAME` / `DROP_CITY_NAME` | City name on the order and opportunity dims and on `PNM_EXPERIENCE`, `PNM_SUPPORT`, `PNM_ALLOCATION`, `PNM_FARE_MOVEMENT`. ✅ **This is the column `leads`/`orders`/`derived`/`p80_durations`/`order_edits` all now use for their city cuts** (`PNM-G-070`, `DECISION_LOG:D21`/`D23`) — confirmed live: `DIM_PNM_OPPORTUNITY.PICKUP_CITY_NAME` holds exactly **14 real city names**, Title Case (`Ahemdabad` is the actual stored spelling, not "Ahmedabad"), plus a genuine `NULL` bucket (5.7% of leads, 0% on the other 4 sections — full city coverage there). `tpo`'s source tables carry NO city column at all (`DIMENSIONS["tpo"]["city_column"]` is `None` by design) — `SHIFTING_REQUIREMENTS.GEO_REGION_ID` → `DIM_GEO_REGIONS` (`PNM-T-007`) is a resolvable path but unvalidated and deprioritized (`DECISION_LOG:D24`). `PICKUP_GEO_REGION_ID` → `DIM_GEO_REGIONS` was not needed for any of the 5 built city cuts; each filters on the city name column directly | `live:INFORMATION_SCHEMA@2026-07-29` (existence), `live@2026-09-22` (values, all 5 city cuts) | **verified** |
| **PNM-T-133** | **Week-bucket mechanics for `leads`' week cut** (`PNM-G-070`, shipped 2026-09-22): Snowflake's `DATE_TRUNC('week', opp_created_ts)` buckets weeks starting **Monday**. Confirmed live for May 2026: 4 of the month's weeks fall entirely inside May, but the leading week (Apr 27–May 3) genuinely spans April and May — **a real, regularly-occurring case, not a rare edge**, since a calendar month rarely starts on a Monday. Summing each week's May-only portion reproduces the exact reconciled monthly total (336,291) — confirms self-consistency, not just plausibility | `owner-ruling:2026-09-22` (boundary-week handling), `live@2026-09-22` | **verified** | A boundary week is answered **in full** (not clipped, not refused), labelled explicitly as spanning two months — `ask.py`'s footer surfaces this via `sqlgen.week_spans_two_months()` |
