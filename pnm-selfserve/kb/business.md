...
| PNM-B-021 | **Every metric in this catalog is monthly by default — but week/day/trend cuts of it are no longer refused.** Superseded twice, same day. | `repo@851886f:pnm-selfserve/selfserve_nlq/metrics_registry.py` | **verified** | ✅ **A single named week or day (`PNM-G-070` close-out, `DECISION_LOG:D25`, 2026-09-22):** nearly every built section (17 of 18, all but `fare`) answers one named week or day in place of the month, same query, narrower grain — one point in time, not a breakdown. ✅ **A weekly/daily TREND (`PNM-G-070` trend close-out, `DECISION_LOG:D27`, 2026-09-22):** the earlier claim that "there is no daily/weekly trend metric anywhere" is now **wrong and superseded** — "weekly orders trend", "daily leads trend" etc. answer a real multi-row breakdown (the same monthly query widened back out and grouped by week/day), for every built section except `fare`. Only `quarterly`/`quarter` remain genuinely unsupported — no quarter grain exists anywhere. Dashboards separately have granularity controls → `PNM-S-011`, `PNM-S-021`, `PNM-S-023`. |
| PNM-B-022 | **The catalog is PnM-wide and mostly cannot be cut by city; `leads` is the sole exception.** City columns exist in the warehouse and the dashboards do filter by city; for `leads`, a city-level query is now self-consistency validated (`PNM-G-070`); every other section still refuses city questions. | `repo@851886f:pnm-selfserve/selfserve_nlq/metrics_registry.py` | **verified** | ⚠ **Was the largest gap between the KB and its audience** — narrowed 2026-09-22 for one section; city ops still gets refused for orders/derived/tpo/p80/edits → `PNM-G-070` stays open |
...
| PNM-B-032 | **Closed-world refusal, with named exceptions for city, week, day, AND trend.** Anything off-menu is refused, never improvised: uncatalogued metrics, region/zone/cluster/tier cuts, unnamed city breakdowns ("leads by city"), quarterly grain, `median`/`p50`/`p90`/`p99`/`average of`, and per-vendor splits everywhere. ✅ **City (`PNM-G-070` pilot, `DECISION_LOG:D21`/`D23`, 2026-09-22): `leads`/`orders`/`derived`/`p80_durations`/`order_edits` answer ONE named city at a time** (e.g. "leads in Bangalore"), gated by a per-section `supports_city` flag — `tpo` and everything else still refuses city (`tpo`'s deprioritized, not built, `DECISION_LOG:D24`). ✅ **Week/day (`PNM-G-070` close-out, `DECISION_LOG:D25`, 2026-09-22): EVERY built section except `fare` answers ONE Monday-start week or ONE specific day at a time** (e.g. "allocation percentage in the week of 2026-05-04", "leads on 2026-05-15") — a mechanical grain swap on the same month query, gated by `supports_week`/`supports_day`. ✅ **Trend (`PNM-G-070` trend close-out, `DECISION_LOG:D27`, 2026-09-22): the SAME sections also answer a full week-by-week or day-by-day TREND** ("weekly orders trend for May 2026", "TPO by week") — the monthly query widened back to the whole month and grouped by grain, a real multi-row answer, not a refusal. `fare` is the one exception throughout: one of its metric-halves reads a column pre-aggregated to month grain on the source mart, not a per-row timestamp, so grain-swapping (narrowing OR widening-and-grouping) it would silently return zero/wrong rows rather than the real answer — it stays month-only, gracefully falling back with a caveat if asked for week/day/trend. ✅ **Dashboard pointer for everything still refused (`DECISION_LOG:D26`, 2026-09-22):** a vendor/stat/unnamed-city-breakdown refusal now names the section's `metabase_fallback` dashboard when the question ties unambiguously to one section (e.g. "vendor wise TPO" → `PNM-S-021`) — silent when the match is ambiguous, never a guessed dashboard. Reuses the same coarse business/ops dashboard split as the city/week fallback, not a per-term-category mapping (`PNM-S-011`/`PNM-S-022`/`PNM-S-023` are more precisely trend-capable per `sources.md` but not individually wired up — moot for trend itself now that it's answered directly rather than refused). | `repo@851886f:pnm-selfserve/selfserve_nlq/metrics_registry.py` (`UNSUPPORTED_TERMS`, `CITIES`, `WEEK_RE`, `DAY_RE`, `TREND_WEEK_TERMS`, `TREND_DAY_TERMS`, `SECTIONS[...]["supports_city"/"supports_week"/"supports_day"/"metabase_fallback"]`) | **verified** | ⚠ The 14 city names are hardcoded; a city absent from the list would be silently answered PnM-wide (unaffected by this change) → `PNM-G-017` (closed, watching `ludhiana`) |
