...
| **PNM-G-024** | **OTA's threshold is now SETTLED; its adoption is not.** | ~~Notion: 30 min + **500 m**. Pipeline: 30 min + **2 km**.~~ **The governed dbt docs define `OTA_FLAG` outright** (`PNM-T-100`): `On_Time` when the order is completed, **`distance_km < 0.5`**, and `shifting_started_event <= shifting_ts + 30 min`. **Notion was right; the pipeline's 2 km was wrong.** ⚠ Two things remain: the rule keys off **shifting-started, not vendor arrival**, so it is not literally an arrival measure; and nothing says `OTA_FLAG` is *sanctioned* for PnM reporting | Owner to rule on **adoption**, not on the threshold: is `OTA_FLAG` the PnM OTA, given it measures shifting-start? A yes unblocks the `ota` section against a governed column | **BLOCKED** — owner *(narrowed 2026-08-26)* |
...
| id | gap | next_action | status |
|---|---|---|---|
| **PNM-G-070** | **The catalog cannot be cut by city or by week, and that is exactly what city ops will ask for.** The columns exist, the dashboards do it, but **no city- or week-level query has ever been reconciled**, so the catalog refuses. **This is the largest gap between this KB and its audience** | Owner/analytics decision: validate a city and weekly variant of the six sections, or route city ops to `PNM-S-020`/`PNM-S-021` permanently and say so out loud. ⚠ **The ground shifted 2026-08-26:** a **governed `pickup_city_name` dimension now exists** on the leads semantic model (`PNM-S-051`), with `relationships` tests pinning `pickup_geo_region_id` → `dim_geo_regions`. For **leads**, a city cut is now a semantic-layer query rather than new work — which changes the cost of this decision materially | **BLOCKED** — owner *(materially changed 2026-08-26)* |
...
| id | gap | conflicting positions | next_action | status |
|---|---|---|---|---|
| **PNM-G-090** | ⚠ **The governed `pnm_overall_leads` metric and this catalog's `leads_overall` DO NOT COMPUTE THE SAME NUMBER — and both carry the owner's name.** | **Governed** (`PNM-S-051`): `count_distinct(opp_id) where user_flag ilike 'normal'`, described as *"across all shifting types"* — **no `shifting_type` filter**. `approved_by: akshay.jain@theporter.in`, 2026-08-11. · **This catalog** (`PNM-M-001`): the same count **restricted to `shifting_type = 'intra_city' OR IS NULL`**, mirroring the MBR automation reconciled at `V3`. **The governed metric is strictly larger** — it includes inter-city, vehicle-shifting and labour leads that the MBR excludes | **DO NOT RESOLVE THIS.** An owner-approved governed metric reads as **rung 1**; the automation mirror is **rung 2** — CONTRIBUTING §6.2 makes that pair the absolute exception. Take both to the owner: *is PnM's canonical lead count intra-city-only (MBR) or all-shifting-types (metric store)?* Whichever wins, the other must be renamed, not silently dropped — **two things called "PnM leads" that differ by the inter-city population is precisely how a leadership number goes wrong** | **BLOCKED** — owner |
...
| G. Coverage | 8 | 1 |
| H. Governed dbt layer vs this catalog | 3 | 1 |
| **Total rows** | **56** | **9** |

*56 rows exist; **1 is CLOSED** (`PNM-G-052`), leaving **55 live** — 46 `OPEN`, 9 `BLOCKED`.*

**The nine owner-blocked rows are the ones that cannot be closed by more analysis:** `PNM-G-003`
...
owner-only-promotes rule so it stops resting on a docstring), `PNM-G-070` (city/weekly cuts — now
**materially cheaper**, since a governed `pickup_city_name` dimension exists), and **`PNM-G-090` —
the governed `pnm_overall_leads` metric and this catalog's `leads_overall` do not compute the same
number, and both carry the owner's name.** That last one is new and is the sharpest of the nine.
