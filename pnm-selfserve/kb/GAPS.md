...
| **PNM-G-018** | **`conversion_others` does not exist**, though `leads_others` and `orders_others` both do. Every other channel has a conversion metric | Ask the owner whether it was omitted deliberately; if not, it is a one-line registry addition | **OPEN** |
...
| **PNM-G-024** | **OTA has two conflicting definitions and no owner** | Notion glossary: within 30 minutes **and a 500 m radius**. Original pipeline metric: within 30 minutes **and 2 km**. Meanwhile `PNM_EXPERIENCE.OTA_FLAG` and `OTA_BREACH_TAT_MINUTES` now exist and encode *someone's* rule, undocumented | Owner to rule: which threshold is correct, and is `OTA_FLAG` the sanctioned source? Until then `ota` stays blocked and OTA questions get this row, not a number | **BLOCKED** — owner |
| **PNM-G-025** | **Recent-month p80 drift is explained but not governed.** p80 is bit-exact for settled months and drifts up to 0.84% for recent ones as the mart backfills. **No policy exists for how old a month must be before its p80 is quotable as final** | Owner to set a settling period (e.g. "p80 is final at M+2"); record it as a `PNM-B-###` row | **BLOCKED** — owner |
...
| **PNM-G-052** | **No mapping exists between the 30/47 registry metric ids and the 167 Argus DD ids.** `resolves_to` is `null` on all 167 rows; the coverage map marks 25 `partial` against a registry that had 30 | Build the id→id mapping, or state explicitly that the two namespaces are not reconcilable and why | **OPEN** |
...
| **PNM-G-062** | **`LMS` is never expanded.** It appears only as a coverage-map metric name (`% of orders contribution — LMS`) and a dashboard card label (`[DBT] Orders View(SCF+LMS)`) | Ask in `#pnm-analytics`; likely a booking system alongside SCF | **OPEN** |
...
owner-only-promotes rule so it stops resting on a docstring), and `PNM-G-070` (city/weekly cuts —
**the biggest one**).
