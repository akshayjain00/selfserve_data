...
- **THREE governed candidates exist, they agree on the 30-minute clock, and they disagree on distance.** ⚠ **REVISED 2026-08-27 — the previous note here said there were two, that both implemented "30 minutes + 500 m", and that they differed on the *event*. All three of those statements were wrong.** `PNM_EXPERIENCE.OTA_FLAG` (`PNM-T-100`) requires `distance_km < 0.5`; `pnm_ota_capacity` (`PNM-T-100a`) requires `ST_DISTANCE(pickup → supervisor GPS) <= 500 m`; `pnm_support.on_time_arrival_flag` (`PNM-T-105`) requires **no distance term at all**. **All three anchor on shifting-started**, so there is no event fork — the "vendor action with GPS" was a *supervisor* `ShiftingStarted` action. The latter two also read the same rows with the same filter and rank, so they are **not independent corroboration** (`PNM-T-105a`). And `distance_km` is **not a documented column** on that mart, while an unused `pickup_km_deviation` is (`PNM-G-098`). ⚠ `pnm_ota_capacity` does publish `ota_percentage` **by city and slot**, which no catalog metric can do — but on a hardcoded 6-city mapping, with absent data published as 0% (`PNM-T-108`, `PNM-T-109`), and "slot" means something different in `pnm_support` (`PNM-T-114`). **The section stays blocked until the owner rules whether OTA requires GPS proximity to the pickup point** → `PNM-G-024`
