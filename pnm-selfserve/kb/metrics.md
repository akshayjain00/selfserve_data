...
- ✅ **City cut, shipped 2026-09-22 (`PNM-G-070` pilot, `DECISION_LOG:D21`).** Ask for one specific named city at a time (e.g. "leads in Bangalore in May 2026") — all 5 metric ids become city-filterable together (same query, `AND pickup_city_name = '<city>'`). **14 recognized cities** (App-side `CITIES` map in `metrics_registry.py`, sourced from `DIM_PNM_OPPORTUNITY.PICKUP_CITY_NAME`, confirmed live): Bangalore, Mumbai, Delhi, Hyderabad, Pune, Chennai, Kolkata, Surat, Lucknow, Coimbatore, Indore, Nagpur, Jaipur, **Ahemdabad** (this exact spelling in the data, not "Ahmedabad" — both question-forms map to it). ⚠ **5.7% of leads have no recorded city** (`PICKUP_CITY_NAME IS NULL`, 19,207 of 336,291 in May 2026) — disclosed, not hidden, same treatment as the `ELSE 'Mobile Website'` channel quirk above. "Leads by city" / "city-wise leads" (a breakdown, not one named city) still refuses everywhere, including here — this pilot answers one city per question, not a `GROUP BY`.
- ✅ **Week cut, shipped 2026-09-22 (`PNM-G-070` pilot, `DECISION_LOG:D21`).** Ask for one specific week by its **Monday start date** (e.g. "leads in the week of 2026-05-04") — replaces the month filter entirely (`DATE_TRUNC('week', opp_created_ts) = '<week_start>'`), same 5 metric ids. ⚠ **A week that spans two calendar months is answered in full, not clipped or refused** — e.g. the week of Apr 27, 2026 (Apr 27–May 3) is a genuine two-month week (confirmed live, not a rare edge case — most months don't start on a Monday). `ask.py` labels a spanning week explicitly in its footer. "Weekly trend" / breakdown-by-week phrasings still refuse everywhere, including here.
- **Both cuts are self-consistency verified, not cross-checked against an external dashboard.** City: the 14 named cities (317,084) + the NULL bucket (19,207) sum to exactly 336,291, the already-reconciled whole-month total. Week: the May-only portion of each week (40,740+67,353+66,158+70,249+91,791) also sums to exactly 336,291. A Metabase cross-check (`PNM-S-020`) was attempted and is not available this session — recorded as a follow-up, not silently treated as done. Live pre-flight and post-ship verification: `DECISION_LOG:D21`/`V10`.
- ✅ **Real city + week cuts shipped 2026-09-22 for `orders`/`derived`/`p80_durations`/`order_edits`, and week-only for `tpo` (`DECISION_LOG:D23`)** — see each section below. `leads` is no longer the only section with a validated cut; `metrics_registry.py`'s new `DIMENSIONS` registry (cites `data-model.md PNM-T-088`) is the single source of truth for where each section's city/week column lives. Only `tpo`'s city remains unbuilt (deprioritized, not infeasible — `DECISION_LOG:D24`).
- ✅ **Week/day generalized to nearly the WHOLE catalog, shipped 2026-09-22 (`PNM-G-070` close-out, `DECISION_LOG:D25`).** Every built section's month filter turned out to already be `DATE_TRUNC('month', <a real per-row timestamp>) = '<value>'`, so answering a single named week or a single day instead is a mechanical grain swap on the SAME query, not new logic — no per-section self-consistency campaign needed the way city required (a real, structural column-per-table problem). **Every built section except `fare` now answers month, week, OR day** — this reaches all the 2026-09-04 sections that were never part of `PNM-G-070`'s original scope too (`ota`, `gac_ctr`, `weekend`, `cac_post_trip`, `vendor_earnings_pctl`, `allocation`, `wallet`, `vendor_tpo_top5`, `addon`, `completion`, `vendor_earnings_bucket`). Ask for a week the same way as before ("… in the week of 2026-05-04") or a day with "… on 2026-05-15". **`fare` is the one deliberate exception**: half its metrics (`total_orders`, `*_surge*`, `*fare_increased*`, `*fare_decreased*`, `orders_with_shifting_started`) are filtered on `order_created_month`, a column pre-aggregated to MONTH GRAIN on the governed `PNM_FARE_MOVEMENT` mart itself — not a timestamp this layer truncates — so grain-swapping it would silently match zero rows instead of erroring; `fare` stays month-only and falls back gracefully (§below) if asked for a week/day.
- ✅ **Graceful fallback, shipped 2026-09-22 (`DECISION_LOG:D22`), now applies only to the two genuinely-unbuilt cases in the whole catalog.** A city/week/day question against a section without that cut does **not** refuse — it drops the unsupported filter, answers that section's plain PnM-wide monthly figure, and prints a `⚠ CAVEAT` naming the dashboard that does have the real cut today where one is configured (`sources.md`: `PNM-S-020` PnM — Business Health, `PNM-S-021` PNM — Operation Dashboard). This now fires only for **`tpo`'s city** and **`fare`'s week/day** — everything else in the catalog answers for real. A week/day request specifically needs a `--month` to fall back to — with none given there is nothing to compute, so it still refuses (naming the dashboard where one is configured). This is a failure-mode improvement, not a substitute for a real cut — the catalog does not query Metabase itself, it names where a human can look right now.
...
- ✅ **City + week cut, shipped 2026-09-22 (`PNM-G-070`/`DECISION_LOG:D23`).** Same contract as `leads`' own cut (§1) — one named city or one Monday-start week at a time — but on `DIM_PNM_ORDERS.PICKUP_CITY_NAME`, the order dim's **own** city column, not the opportunity dim's `leads` uses. **100% city coverage** (0 NULL, unlike `leads`' 5.7%). Self-consistency verified live: city split summed to 51,276, exact match to the live May 2026 total; week split (incl. the Apr27-May3 boundary week) also 51,276 exact.
...
- ✅ **City + week cut, shipped 2026-09-22 (`PNM-G-070`/`DECISION_LOG:D23`), every metric id in this section.** Same filter applied to both the underlying `leads_cte` and `orders_cte` — not independently re-verified as its own query, since it inherits `leads`' and `orders`' own already-validated city/week cuts and the ratio is computed from those raw counts exactly as the unfiltered version always was.
...
- ✅ **Week cut ONLY (no city), shipped 2026-09-22 (`PNM-G-070`/`DECISION_LOG:D23`), every metric id in this section.** One Monday-start week at a time, replacing the month filter on both the `orders`/`tickets` CTEs' own timestamp (`orders`: allocation completion; `tickets`: ticket creation) — same shape as the month cut, just truncated to `week`. **No city cut** — this section's source tables carry no city column at all; a path exists (`SHIFTING_REQUIREMENTS.GEO_REGION_ID → DIM_GEO_REGIONS`, `PNM-T-007`) but is unvalidated (FK reliability unconfirmed), so it was deliberately not built. Only `orders_base` (a `COUNT`) was self-consistency verified this way — split by week (incl. the Apr27-May3 boundary's May-only portion) summed to 45,414, exact match to both the live monthly total and the already-reconciled `business.md` anchor; the `tpo_*` ratios inherit the same week filter but are not independently additive, same as the month cut never was.
...
- ✅ **City + week cut, shipped 2026-09-22 (`PNM-G-070`/`DECISION_LOG:D23`), every metric id in this section.** On this same mart's `PICKUP_CITY_NAME` / `SHIFTING_TS_IST`. ⚠ **Verified at the POPULATION level only, not the metric value** — a percentile isn't additive across a split the way a `COUNT` is, so only the row-count split was self-consistency checked (45,208 exact, both for city and for the week split incl. the Apr27-May3 boundary) — confirming the filter behaves correctly (right orders, no silent drops), not that the resulting `p80` figure has an independent cross-check. Treat this section's city/week answers with that in mind — weaker evidentiary footing than a COUNT-based section like `orders`.
...
- ✅ **City + week cut, shipped 2026-09-22 (`PNM-G-070`/`DECISION_LOG:D23`), every metric id in this section.** On this same mart's `PICKUP_CITY_NAME` / `ORDER_CREATED_TS_IST` (this section's own month_basis column, not `SHIFTING_TS_IST` — `p80_durations` and `order_edits` truncate on different columns even though they share a table). City split summed to 43,558 exact (0 NULL); week split (incl. the Apr27-May3 boundary) also 43,558 exact. `no_of_successful_edits` (a `SUM`) is genuinely additive and covered by that check; every `pct_*` metric is a ratio of counts — same non-additive caveat as `derived`, only the underlying population is verified, not the ratio itself.
